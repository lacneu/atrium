...
| CC6.1 — Secrets in env | Auth keys + server-to-server secrets never in DB | Bootstrap script pushes to Convex deployment env | `deploy/compose/bootstrap-env.sh`, `deploy/helm/templates/bootstrap-job.yaml` | Use a secrets manager; rotate secrets | PRODUCT / OPERATOR |
| CC6.1 — Secret encryption at rest | Gateway credentials stored AES-256-GCM-encrypted (authenticated envelope, context-bound via AAD, crypto-agile / KMS-ready); encrypt/decrypt only in actions; plaintext never in DB or browser | Master key in deployment env (`ATRIUM_SECRET_KEY`) | `convex/lib/crypto/`, `convex/instanceSecrets.ts`, `convex/bridge_credentials.ts` | Set + back up `ATRIUM_SECRET_KEY`; consider an external KMS | PRODUCT |
| CC6.1 — Per-bridge credential isolation | Credential fetch authenticated by a per-bridge secret (hashed at rest) resolving to exactly one instance; a bridge fetches only its own credentials | `bridgeAuth.by_hash`; response `Cache-Control: no-store` | `convex/bridge_credentials.ts`, `convex/bridgeAuth.ts` | Mint a distinct secret per bridge; rotate/revoke per bridge | PRODUCT |
