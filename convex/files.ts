...
export const listMine = query({
  args: {
    ...
    // Auto-generated files (origin "pasted", the composer's large-paste guard)
    // are HIDDEN unless this is true — the default listing shows real files.
    includeAutoGenerated: v.optional(v.boolean()),
  },
  handler: async (ctx, args) => {
    ...
    if (args.includeAutoGenerated !== true) {
      // Default view = REAL files only: auto-generated rows (paste-as-file) are
      // excluded. Residual filter (not indexed): "pasted" rows are a small
      // minority, so the extra scan to fill the cap is marginal — and when a
      // user has ONLY pasted files, an empty default listing is the correct,
      // honest answer (the toggle reveals them).
      listQuery = listQuery.filter((f) =>
        f.eq(f.field("origin"), undefined),
      );
    }
