...
| `BRIDGE_FORWARDED_CLIENT_IP` | no | *first routable address of the host* | Address the gateway attributes this bridge's connections to, in `trusted-proxy` mode. The bridge is the client here — this is not the end user's browser, which the bridge never learns. A loopback value is refused at startup: the gateway rejects the upgrade outright unless the forwarded headers name a routable address. | `10.0.3.14` |
