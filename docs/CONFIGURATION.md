...
2. **Convex _deployment_ env** — read by Convex **functions** via `process.env`.
...
- **Example** — anonymised and syntactically valid. Never copy a secret from a
  document; generate your own (see _Secrets discipline_).
...
| `OPENCLAW_GATEWAY_VERSION`            | no (legacy loader only) |                                      | Declares the gateway version instead of discovering it. Only for a gateway whose version probe is unavailable; a wrong value silently disables version-gated features. **Read only by the single-instance legacy loader.** The deployed container resolves every instance from Convex, so set the gateway version on the instance in Atrium (Settings → Agents → Bridge) instead; this variable has no effect there.                                                                                                                                                                                                                                                                                           | `2026.7.1`                                    |
...
| `OPENCLAW_AUTH_MODE`                  | no                      | `token`                              | How this bridge authenticates to its gateway. `token` is the shared operator credential: the gateway attributes every session to one owner profile, so anyone holding the token sees all of them. `trusted-proxy` makes the bridge state WHO each socket acts for, and the gateway resolves one user profile per person. Only `trusted-proxy` opts in; any other value keeps `token`. The gateway must be configured for the same mode — the two are mutually exclusive there, and a gateway in trusted-proxy mode refuses to hold a token at all. **Read only by the single-instance legacy loader**; the deployed container sets the mode per instance in Atrium.                                            | `trusted-proxy`                               |
| `OPENCLAW_SYSTEM_IDENTITY`            | no                      | `atrium-bridge:<instance>`           | Identity presented on sockets that serve no single person — agent discovery, transcript recovery after a crash, config defaults. Keeps its full grant on purpose: recovery must be able to read a session created by someone else. Ignored in `token` mode.                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `atrium-bridge:alpha`                         |
| `BRIDGE_FORWARDED_CLIENT_IP`          | no                      | _first routable address of the host_ | Address the gateway attributes this bridge's connections to, in `trusted-proxy` mode. The bridge is the client here — this is not the end user's browser, which the bridge never learns. A loopback value is refused at startup: the gateway rejects the upgrade outright unless the forwarded headers name a routable address.                                                                                                                                                                                                                                                                                                                                                                                | `10.0.3.14`                                   |
| `OPENCLAW_TRUSTED_PROXY_USER_HEADER`  | no                      | `x-forwarded-user`                   | Header the gateway reads the identity from, when it is configured with something other than the default (`gateway.auth.trustedProxy.userHeader`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `x-forwarded-user`                            |
...
| `OPENCLAW_MEDIA_OUTBOUND_DIR`         | no                      | _derived from the instance name_     | **Process-global override** of the bridge's outbound media dir. Use it only when the bridge serves exactly ONE instance. With several, it collapses every instance onto one path — see _Media paths_ below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `/home/node/.openclaw/media/alpha/outbound`   |
| `OPENCLAW_INBOUND_DIR`                | no                      | _derived from the instance name_     | **Process-global override** of the bridge's one inbound root mount. The bridge derives `<root>/published` and `<root>/.staging`; use it only for a single-instance bridge. The removed `OPENCLAW_INBOUND_STAGING_DIR` is rejected.                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `/home/node/.openclaw/media/alpha/inbound`    |
| `OPENCLAW_INBOUND_STAGING_DIR`        | forbidden               |                                      | Removed compatibility input. The bridge reads it only to reject startup: a separate staging mount breaks the same-filesystem atomic rename guarantee. Use one `OPENCLAW_INBOUND_DIR` root containing `published/` and `.staging/`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |                                              |
...
| `OPENCLAW_ATTACHMENT_FIX_ATTESTED`    | no                      | _(none)_                             | Instances whose gateway image carries the fix for the 2026.8.1/2026.8.2 attachment defect, comma separated, or `*` for all. On those two versions a delivered file poisons the session, so the bridge stops telling the agent how to deliver one; naming an instance here restores the instruction for that instance only. A value naming no instance attests nothing. **This lifts the BRIDGE's own guard.** Convex applies the same quarantine independently, and reads its own attestation from the instance's config (Settings → Agents → Bridge, `attachmentFixAttested`) — an unauthenticated `/capabilities` answer cannot authorize it. Set BOTH to restore file delivery on a patched 2026.8.x image. | `alpha,beta`                                  |
...
| `JWT_PRIVATE_KEY`                       | yes             |               | PKCS#8 private key signing session JWTs. Generate with `node deploy/compose/generate-auth-keys.mjs`.                                                              | _(PEM block)_                                     |
| `JWKS`                                  | yes             |               | Public JWKS matching the key above. Generated by the same script — the pair must come from one run.                                                               | _(JSON)_                                          |
...
| `BRIDGE_SHARED_SECRET` | yes      |         | Must equal the bridge's.                                                                 | _(same value)_       |
| `BRIDGE_INGEST_SECRET` | yes      |         | Must equal the bridge's.                                                                 | _(same value)_       |
...
| `SIGNED_ANNOUNCEMENTS_PUBLIC_KEY`      | no       |         | Key verifying the signature. An announcement that fails verification is dropped, never displayed. | _(base64)_                     |
| `SIGNED_ANNOUNCEMENTS_KEY_MAP`         | no       |         | Several public keys by id, for rotation without a gap.                                            | _(JSON)_                       |
