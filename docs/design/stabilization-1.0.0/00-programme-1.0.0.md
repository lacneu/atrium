...
| W9 | Auto-découverte des trames non traitées | **tranches 1 et 2a livrées** — lots 23 et 28 (points 6 et 8 : `KNOWN_AGENT_FIELDS` dérivé du vendoring, drift par état, débordements comptés bout en bout, auto-échec du détecteur ; puis le capteur d'exception C4 sur les sept lecteurs). **Le point 4 est livré au lot 38** — la table `protocolShapes` existe (`convex/schema.ts:1453`). **Le point 7 (G-70) est livré au lot G-70** : le catalogue annoncé par la passerelle est vendorisé, classé, et gardé par un cliquet CI ; le handshake ne compte que ce que le contrat n'avait jamais anticipé ; la moitié Hermes classe les 24 capacités de `/v1/capabilities`. **Reste la tranche 2b** : points 1, 3, 5, 9, 10 |
...
| G-70 **(FERMÉE — lot G-70)** | Le gateway **donne déjà** la liste de ce qu'il émet (`hello-ok.features.events`, `GET /v1/capabilities`) et Atrium ne la lisait pas. Mesure : 30 familles annoncées, **4 réellement alimentées**, 26 non lues (la passe 4 de revue a corrigé `session.operation` : lecteur présent, jamais alimenté) — d'où une classification exhaustive gardée par cliquet plutôt qu'une assertion de sous-ensemble rouge à la naissance. Côté Hermes l'asymétrie est dite : les capacités étaient déjà LUES, c'est la classification qui manquait (24 déclarées, 19 offertes, **12 inconsommées** — les passes 4 et 5 ont trouvé six classifications qui affirmaient une consommation inexistante). **Ne ferme PAS G-69** : la dérive à l'exécution et le comptage des trames non routées restent à G-58 tranche 2+ | H | 8 | `$UP/packages/gateway-protocol/src/schema/frames.ts:99-103` peuplé par `$UP/src/gateway/server.impl.ts:1581` ; hello-ok lu mais `features` inexploité `openclaw-client.ts:312-318` ; Hermes déjà appelé `hermes/client.ts:152` | W9 |
